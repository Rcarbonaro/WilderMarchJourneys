...
const PARAGRAPHS: Array[Dictionary] = [
	{ "text": "Long ago, Elyndra's Prophecy warned of The Forsaken Hour, an apocalyptic age where the gods would abandon their creation. In that time, the horrors from beyond would corrupt the life granted by the gods..."},
	{ "text": "It was said that if we remained faithful, Elyndra would appear in the Last Days to save us. For many years, the Inari fought hard to keep Elyndra's tenants, believing their brutal piety would save the world…" },
	{ "text": "However, as the Rodescian Empire grew in strength, the Inari could not stand against them. The influence of the Inari, and of Elyndra’s prophecy quickly dwindled as the Rodescians became seen as the primary existential threat to the East…" },
