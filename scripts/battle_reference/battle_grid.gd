...
func spawn_scatter_features(feature_placements: Array) -> void:
	# Renders every decorative/blocking/slowing object MapGenerator placed.
	# REQUIRES a Node2D child of BattleGrid named "FeatureLayer", positioned
	# right after GroundLayer in the scene tree, so scattered objects render
	# above bare ground but below auras/hazards/units. This node has to be
	# added once in the Godot editor -- it can't be created from a script.
	if not has_node("FeatureLayer"):
		printerr("⚠️ BattleGrid: no 'FeatureLayer' node found -- add a Node2D ",
				 "named FeatureLayer right after GroundLayer in BattleGrid.tscn.")
		return
	for placement in feature_placements:
		var cell: Vector2i = placement["cell"]
		var feature: MapFeatureData = placement["feature"]
		var visual: Node2D
		if feature.scene != null:
			visual = feature.scene.instantiate()
		else:
			var sprite := Sprite2D.new()
			sprite.texture = feature.texture
			visual = sprite
		var base_pos := grid_to_world(cell)
		if feature.visual_offset_jitter_px > 0.0:
			base_pos += Vector2(
				randf_range(-feature.visual_offset_jitter_px, feature.visual_offset_jitter_px),
				randf_range(-feature.visual_offset_jitter_px, feature.visual_offset_jitter_px)
			)
		visual.position = base_pos
		if feature.random_scale_variance > 0.0:
			var s := 1.0 + randf_range(-feature.random_scale_variance, feature.random_scale_variance)
			visual.scale = Vector2(s, s)
		$FeatureLayer.add_child(visual)
